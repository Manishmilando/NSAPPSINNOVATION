...
const ServiceDetail = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (document.querySelector(".team-floater")) {
  ...
  const stackImages =
    service.images && service.images.length >= 3
      ...
      ? [...service.images, service.img, service.img].slice(0, 3)
      : [service.img, service.img, service.img];

  // Marquee content repeated for seamless loop
  const marqueeText =
    " INNOVATION • STRATEGY • SCALABILITY • PERFORMANCE • RELIABILITY • SOLUTIONS • ";

  // Generate gallery items from service data
  const getGalleryItems = () => {
    ...
    const spanPatterns = [
      ...
      "col-span-1 sm:col-span-2 md:col-span-2 row-span-1", // Wide
      "col-span-1 sm:col-span-1 md:col-span-1 row-span-1", // Small
      "col-span-1 sm:col-span-1 md:col-span-1 row-span-1", // Small
      "col-span-1 sm:col-span-1 md:col-span-1 row-span-2", // Tall
    ];

    // Map service images to gallery items
    return service.images.map((imageSrc, index) => ({
      type: "image",
      src: imageSrc,
    ...
    >
      ...
      <header className="relative w-full max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 pt-20 sm:pt-24 md:pt-32 pb-12 sm:pb-16 md:pb-24 min-h-[80vh] md:min-h-[90vh] flex flex-col lg:flex-row items-center justify-between gap-8 sm:gap-10 md:gap-12 z-10">
        ...
        <div className="w-full lg:w-1/2 order-1 lg:order-2 relative h-[400px] sm:h-[800px] md:h-[600px] flex items-center justify-center lg:justify-end">
          {/* 3D Card Stack - Inline with Service Images */}
        ...
        <div className="w-full lg:w-1/2 order-2 lg:order-1 max-w-2xl relative z-20">
          <div className="overflow-hidden mb-3 sm:mb-4 md:mb-6">
            <div className="hero-text-mask inline-block">
              <span className="text-xs sm:text-sm font-bold tracking-[0.15em] sm:tracking-[0.2em] text-gray-500 uppercase flex items-center gap-2 sm:gap-3">
                {service.client || "Expert Services"}
              </span>
            </div>
          </div>
          ...
          <div className="hero-text-mask flex gap-3 sm:gap-4">
            <button
              onClick={() =>
                service.link && window.open(service.link, "_blank")
              }
      ...
      {/* RESPONSIVE BENTO GRID - DYNAMIC FROM SERVICE DATA */}
